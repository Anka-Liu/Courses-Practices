...
% Last Modified by GUIDE v2.5 07-Jul-2018 16:46:04
...
if strcmp(handles.popupmenu2.String{handles.popupmenu2.Value},'All') %if opt to show all cities
    set(handles.popupmenu3,'String',unique({'All',state1.Name})); %load airports into menu
    city1=state1;
else   
    fun=cellfun(@(x) strcmp(x,handles.popupmenu2.String{handles.popupmenu2.Value}),{state1.City});
    city1=state1(fun);%if city specified, search corresponding airports
    set(handles.popupmenu3,'String',unique({'All',city1.Name})); %load airports into menu
end

%it is the same idea to set the menus in Route Tracking page
%the starting airport menu set
global state2 city2 state3 city3;
...
state2=S(fun);
set(handles.popupmenu5,'String',unique({state2.City}));
fun=cellfun(@(x) strcmp(x,handles.popupmenu5.String{handles.popupmenu5.Value}),{state2.City});
city2=state2(fun);
set(handles.popupmenu6,'String',unique({city2.Name}));
...
state3=S(fun);
set(handles.popupmenu8,'String',unique({state3.City}));
fun=cellfun(@(x) strcmp(x,handles.popupmenu8.String{handles.popupmenu8.Value}),{state3.City});
city3=state3(fun);
set(handles.popupmenu9,'String',unique({city3.Name}));
...
global city2 city3; %use database
...
selectS=cellfun(@(x) strcmp(x,handles.popupmenu6.String(handles.popupmenu6.Value)),{city2.Name}); %search data of departure
LoS=[city2(selectS).Longitude]; %take its longitude
LaS=[city2(selectS).Latitude]; %take its latitude
selectD=cellfun(@(x) strcmp(x,handles.popupmenu9.String(handles.popupmenu9.Value)),{city3.Name}); %search data of destination
LoD=[city3(selectD).Longitude]; %take its longitude
LaD=[city3(selectD).Latitude]; %take its latitude
XS=-(cos(LaS(1)./180.*pi).*cos(LoS(1)./180.*pi)); %calculate x-coordination of departure
YS=-(cos(LaS(1)./180.*pi).*sin(LoS(1)./180.*pi));%calculate y-coordination of departure
ZS=sin(LaS(1)./180.*pi);%calculate z-coordination of departure
XD=-(cos(LaD(1)./180.*pi).*cos(LoD(1)./180.*pi)); %calculate x-coordination of destination
YD=-(cos(LaD(1)./180.*pi).*sin(LoD(1)./180.*pi)); %calculate y-coordination of destination
ZD=sin(LaD(1)./180.*pi); %calculate z-coordination of destination
...
global cl j;%mark whether the program is closed
cl=0;%mark the oped/end of program
set(handles.text36,'String','Done.') %change text from Loading to Done
 %while the stop button is not pushed down
    %by frequently changing the position of camera, we make earth rotate.

 while cl==0 && handles.togglebutton3.Value==0 
        for i=linspace(0,2*pi,501)
            if cl==0 && handles.togglebutton3.Value==0 %if program not closed
                j=i;%record position of earth
...
pause(0.1);
global S state1 city1;% use database
...
set(handles.popupmenu2,'String',unique({'All',state1.City})); %load names of cities of corresponding country
fun=cellfun(@(x) strcmp(x,handles.popupmenu2.String{handles.popupmenu2.Value}),{state1.City}); %search airports of corresponding city
city1=state1(fun);
set(handles.popupmenu3,'Value',1); %set option back to first of the list
set(handles.popupmenu3,'String',unique({'All',state1(fun).Name})); %load corresponding airport names
...
if strcmp(handles.popupmenu2.String{handles.popupmenu2.Value},'All') %if select "All"
    ...
    set(handles.popupmenu3,'String',unique({'All',state1.Name})); %load names into third popupmenu
    city1=state1;
else   
    fun=cellfun(@(x) strcmp(x,handles.popupmenu2.String{handles.popupmenu2.Value}),{state1.City});%search the names of airports of corresponding city
    city1=state1(fun);
    set(handles.popupmenu3,'Value',1); %set option index back to first of the list
    set(handles.popupmenu3,'String',unique({'All',city1.Name})); %load names into third popupmenu
end
set(handles.text23,'String','Ready.')
...
global state1 city1; %use database
...
if strcmp(handles.popupmenu3.String{handles.popupmenu3.Value},'All') && ...
    ...
    Lo=[state1.Longitude]; %extract Longitude 
    La=[state1.Latitude]; %extract Latitude
...
elseif strcmp(handles.popupmenu2.String{handles.popupmenu2.Value},'All') && ...
    ...
    select=cellfun(@(x) strcmp(x,handles.popupmenu3.String{handles.popupmenu3.Value}), {state1.Name});
      %the same as above, convert coordinates of airports and plot them
    Lo=[state1(select).Longitude];
    La=[state1(select).Latitude];
    X=-(cos(La./180.*pi).*cos(Lo./180.*pi));
    Y=-(cos(La./180.*pi).*sin(Lo./180.*pi));
    Z=sin(La./180.*pi);
    hold(ax5,'on');
    ...
    select=cellfun(@(x) strcmp(x,handles.popupmenu3.String{handles.popupmenu3.Value}), {city1.Name});
    %the same as above, convert coordinates of airports and plot them
    Lo=[city1(select).Longitude];
    La=[city1(select).Latitude];
    X=-(cos(La./180.*pi).*cos(Lo./180.*pi));
    Y=-(cos(La./180.*pi).*sin(Lo./180.*pi));
    Z=sin(La./180.*pi);
    hold(ax5,'on');
...
pause(0.1);
global S state2 city2; %use database
...
global cl j; %mark closure of program

if cl==0 && handles.togglebutton3.Value==0
for i=linspace(j,2*pi,501) %continuing the stopped rotation
            if cl==0 && handles.togglebutton3.Value==0%if pressed stop
                j=i;%record position
...
while cl==0 && handles.togglebutton3.Value==0 %check the button to decide whether to loop 
        for i=linspace(0,2*pi,501)
            if cl==0 && handles.togglebutton3.Value==0%if pressed stop
                j=i;%record position
                    handles.axes1.CameraPosition=[10*sqrt(3)*cos(i) 10*sqrt(3)*sin(i) 0]; %roll position of earth
